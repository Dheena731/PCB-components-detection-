...
# Directory where YOLO models are stored (Windows path)
MODEL_DIR = "D:\\Krishtec\\models\\senior board for jetson yolov8n"  # Adjusted for Windows
MODEL_PATH = os.path.join(MODEL_DIR, "best.pt")  # Single model file assumed
...
def load_model():
    ...
    try:
        ...
        log_message(f"Loaded model: {selected_model} on {device}")
...
def start_component_setup():
    ...
    def capture_setup_frame():
        ...
        try:
            results = model(frame, conf=confidence_threshold, device=device)  # Use GPU if available
...
def start_realtime():
    ...
    capture_paused = False
    if cap is None:
        cap = cv2.VideoCapture(0)  # Default webcam on Windows
